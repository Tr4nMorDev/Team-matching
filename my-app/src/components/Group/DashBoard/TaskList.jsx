...
const TaskList = () => {
    ...
    (async () => {
      try {
        const response = await axios.get(
          `${API_PROJECT}/api/tasks/team/${teamId}`
        );
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.put(
        `${API_PROJECT}/api/tasks/${selectedTask.id}/status?leaderId=${leaderId}`,
        {
  ...
  const handleDelete = async (taskId) => {
    try {
      ...
      await axios.delete(`${API_PROJECT}/api/tasks/${taskId}?userId=${userId}`);
  ...
  return (
    <article className="flex flex-col md:flex-row md:items-center bg-white rounded-xl p-6 shadow-md hover:shadow-lg transition-shadow duration-300 relative border border-gray-200">
      <div className="flex-1">
        <h2
          className="text-lg font-semibold text-gray-900 mb-2 md:mb-1"
          title={task.taskName}
        >
          {task.taskName}
        </h2>
        <p className="text-gray-700 mb-4 md:mb-2 leading-relaxed">
          {task.description}
        </p>
