    ...
    const fetchMails = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_PROJECT}/api/lecturer-join-requests/pending/${lecturerId}`,
          {
            headers: {
  ...
  const handleRespond = async (requestId, action) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_PROJECT}/api/lecturer-join-requests/respond?${params.toString()}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
              ...
              mails.map((mail) => (
                ...
                >
                  <img
                    src={
                      mail.teamPicture ||
                      "https://placehold.co/48x48/png?text=Team"
                    }
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-gray-900 font-semibold truncate">
                      Group:{" "}
                      <span className="text-blue-600">{mail.teamName}</span>
                    </p>
                    <p className="text-gray-500 text-sm truncate">
                      From leader: {mail.leaderName}
                    </p>
