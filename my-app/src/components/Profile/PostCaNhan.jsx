...
const PostCaNhan = ({ userId }) => {
  ...
  useEffect(() => {
    ...
    const fetchBlogs = async () => {
      try {
        const res = await axios.get(`http://localhost:8080/api/blogs/user/${idToUse}`);
        setBlogs(res.data);
      } catch (err) {
  ...
  return (
      <>
        {blogs.map((blog) => (
            <div key={blog.id} className="p-4 mt-4 bg-white rounded-lg shadow relative">
              {/* Header */}
                <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <img
                      src={blog.authorAvatar || "/avata.jpg"}
                      alt="Profile"
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-900">
                      {blog.authorName}
                      {isOwnPage && <span className="ml-1 text-blue-500">(Bạn)</span>}
                    </h3>
