...
const FriendsList = ({ userId: propUserId }) => {
    ...
    const fetchFriends = async () => {
      ...
      try {
        const response = await axios.get(
          `${API_PROJECT}/api/friends/list/${targetUserId}`,
          {
        ...
        if (targetUserId !== currentUser.id) {
          const friend = response.data.find(
            (f) =>
              (f.requester.id === currentUser.id ||
                f.receiver.id === currentUser.id) &&
              f.status === "ACCEPTED"
          );
          setIsFriend(!!friend);
        }
  ...
  const handleUnfriend = async (friendId) => {
    ...
    try {
      await axios.delete(`${API_PROJECT}/api/friends/${friendId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSendFriendRequest = async () => {
    ...
    try {
      await axios.post(
        `${API_PROJECT}/api/friends/request/${currentUser.id}/${targetUserId}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="p-6 bg-white shadow-md rounded-lg w-full max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4 mt-4">
        {friends.map((friend) => {
          const displayFriend =
            friend.requester.id === targetUserId
              ? friend.receiver
              : friend.requester;
